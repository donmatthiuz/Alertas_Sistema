...
import comercios from '../mockups/data_comercio'

export default function ModalVincular({open, setOpen}) {
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);

  const [comercios_, setComercios] = useState([])
      ...
      useEffect(() => {
        ...
        const options_cast = () => {
          const comercioOptions = comercios.map((comercio) => (
            comercio.nombre
          ))


          console.log(comercioOptions)
            
            
          setComercios(comercioOptions)
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <DialogTitle as="h1" className="text-base font-semibold text-gray-900">
                      Asignar Comercio a POS <Badge variant='warning'>N#</Badge>
                    </DialogTitle>
                    <div className="mt-2">
                      <p className="text-lg text-gray-500">
                        Si no encuentras el comercio que buscas agregalo
                      </p>
                    </div>
                    
                    <div className={`mt-4 ${isDropdownOpen ? 'pb-16' : 'pb-4'} transition-all duration-300`}>
                      <SearchDropdownOutlined
                        placeholder={"Buscar Comercio"}
                        initialOptions={comercios_}
